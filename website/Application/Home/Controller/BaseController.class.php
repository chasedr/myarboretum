...
class BaseController extends Controller {
	...
	public function getplants()
	{
		...
		if($sessionid != null)
		{
			...
			{
				array_push($plantlist,$item['selfspace_plantunit_name']);
			}
			#var_dump($plantlist);
			$this->ajaxReturn($plantlist,'JSON');
		}
		else
		{echo "session set failure.";}
	}
	...
	public function getplants2()
	{
		...
		if($sessionid != null)
		{
			$selfspace = M("selfspace");
			#$result = $selfspace->where('selfspace_user_id = \''.session('useropenid').'\'')->group('selfspace_plantunit_name')->select();
			$result = $selfspace->where('selfspace_user_id = \'oJk0E0WSPdGInBAHlI0uSZNeF608\'')->group('selfspace_plantunit_name')->select();
			#echo "return";
			#print_r(session('useropenid'));
			#print_r($result);
			$plantlist = array();
			foreach($result as $item)
			{
				...
				$array = array(
					"plantpicpath"=>md5($item['selfspace_user_id']).'/'.md5($item['selfspace_plantunit_name']).'/'.md5($item['selfspace_picname']).'.'.end($filename),
					"selfspace_plantunit_name"=>$item["selfspace_plantunit_name"],
					"selfspace_inserttime"=>$item["selfspace_inserttime"],
					"selfspace_describe"=>mb_substr($item["selfspace_describe"],0,6,'utf-8')
				);
				array_push($plantlist,$array);
			}
			#var_dump($plantlist);
			$this->ajaxReturn($plantlist,'JSON');
		}
	...
	public function getplants3()
	{
		$plantunitname = I("post.selfspaceplantunitname");
		#print_r($plantunitname);
		$selfspace = M("selfspace");
		$result = $selfspace->where('selfspace_user_id = \'oJk0E0WSPdGInBAHlI0uSZNeF608\' and selfspace_plantunit_name='.'\''.$plantunitname.'\'')->select();	
		$plantlist = array();
		foreach($result as $item)
		{
		         #print_r($item['selfspace_user_id']);
